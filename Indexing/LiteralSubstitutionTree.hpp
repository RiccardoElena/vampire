...
private:
  ...
  auto getResultIterator(Literal *lit, bool complementary, bool retrieveSubstitutions, Args... args)
  {
    ...
    auto iter = [tree, lit, retrieveSubstitutions, &args...](bool reversed) { return tree->template iterator<Iterator>(lit, retrieveSubstitutions, reversed, args...); };

    return ifElseIter(
        tree->isEmpty(), [&]() { return VirtualIterator<ELEMENT_TYPE(Iterator)>::getEmpty(); },
        [&]() { return ifElseIter(!lit->isEquality(), [&]() { return iter(/* reverse */ false); }, [&]() { return concatIters(iter(/* reverse */ false), iter(/* reverse */ true)); }); });
    // if (tree.isEmpty()) {
    //
    // }
