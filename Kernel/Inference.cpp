...
void Inference::destroy()
{
  switch (_kind) {
    case Kind::INFERENCE_012:
      if (_ptr1)
        static_cast<Unit *>(_ptr1)->decRefCnt();
      if (_ptr2)
        static_cast<Unit *>(_ptr2)->decRefCnt();
      break;
    case Kind::INFERENCE_FROM_SAT_REFUTATION:
      delete static_cast<FromSatRefutationInfo *>(_ptr2);
      // intentionally fall further
...
std::string Kernel::ruleName(InferenceRule rule)
{
  switch (rule) {
    ...
    case InferenceRule::SEPARATION:
      return "separation";
    case InferenceRule::FLUTED_RESOLUTION:
      return "fluted resolution";
    ...
    case InferenceRule::INTERNAL_THEORY_AXIOM_LAST: { /* explicitly ignoring this cases */
    }
  }
