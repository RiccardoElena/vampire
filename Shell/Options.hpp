...
private:
  ...
  struct AbstractOptionValue {
    AbstractOptionValue() {}
    AbstractOptionValue(std::string l, std::string s) : longName(l), shortName(s), experimental(false), is_set(false), _should_copy(true), _tag(OptionTag::LAST_TAG), supress_problemconstraints(false) {}
    ...
    void tag(OptionTag tag)
    {
      ASS(_tag == OptionTag::LAST_TAG);
      _tag = tag;
    }
  ...
  struct OptionValue : public AbstractOptionValue {
    ...
    void addHardConstraint(OptionValueConstraintUP<T> c)
    {
      c->setHard();
  ...
  struct ChoiceOptionValue : public OptionValue<T> {
    ChoiceOptionValue() {}
    ChoiceOptionValue(std::string l, std::string s, T def, OptionChoiceValues c) : OptionValue<T>(l, s, def), choices(c) {}
      ...
      for (int i = 0; i < choices.length(); i++) {
        if (i == 0) {
          ...
          if (linewrap && next.size() + count > 60) { // next.size() will be <70, how big is a tab?
            out << std::endl
                << "\t";
            for (unsigned j = 0; j < values_header.size(); j++) {
              out << " ";
            }
            count = 0;
          }
  ...
  struct RatioOptionValue : public OptionValue<int> {
    RatioOptionValue() {}
    RatioOptionValue(std::string l, std::string s, int def, int other, char sp = ':') : OptionValue(l, s, def), sep(sp), defaultOtherValue(other), otherValue(other){};
  ...
  struct NonGoalWeightOptionValue : public OptionValue<float> {
    ...
    NonGoalWeightOptionValue(std::string l, std::string s, float def) : OptionValue(l, s, def), numerator(1), denominator(1){};
  ...
  struct SelectionOptionValue : public OptionValue<int> {
    ...
    SelectionOptionValue(std::string l, std::string s, int def) : OptionValue(l, s, def){};
  ...
  struct InputFileOptionValue : public OptionValue<std::string> {
    ...
    InputFileOptionValue(std::string l, std::string s, std::string def, Options *p) : OptionValue(l, s, def), parent(p){};
  ...
  struct DecodeOptionValue : public OptionValue<std::string> {
    ...
    DecodeOptionValue(std::string l, std::string s, Options *p) : OptionValue(l, s, ""), parent(p) { AbstractOptionValue::_should_copy = false; }
  ...
  struct TimeLimitOptionValue : public OptionValue<int> {
    ...
    TimeLimitOptionValue(std::string l, std::string s, float def) : OptionValue(l, s, def){};
  ...
  struct LessThan : public OptionValueConstraint<T> {
    ...
    {
      if (_orequal)
        return value.longName + "(" + value.getStringOfActual() + ") is less than or equal to " + value.getStringOfValue(_goodvalue);
  ...
  struct GreaterThan : public OptionValueConstraint<T> {
    ...
    {
      if (_orequal)
        return value.longName + "(" + value.getStringOfActual() + ") is greater than or equal to " + value.getStringOfValue(_goodvalue);
  ...
  struct SmallerThan : public OptionValueConstraint<T> {
    ...
    {
      if (_orequal)
        return value.longName + "(" + value.getStringOfActual() + ") is smaller than or equal to " + value.getStringOfValue(_goodvalue);
  ...
  struct IfThenConstraint : public OptionValueConstraint<T> {
    IfThenConstraint(OptionValueConstraintUP<T> ic, OptionValueConstraintUP<T> c) : if_con(std::move(ic)), then_con(std::move(c)) {}
  ...
  struct ManyOptionProblemConstraints : OptionProblemConstraint {
    ...
    bool check(Property *p)
    {
      ...
      while (it.hasNext()) {
        bool n = it.next()->check(p);
        res = is_and ? (res && n) : (res || n);
      }
      return res;
    }
