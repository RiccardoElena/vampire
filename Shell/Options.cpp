...
void Options::init()
{
  ...
  _mode.description =
      ...
      "  -fluted: classify and resolve problem in the fluted fragment\n"
  ...
  /*output for fluted fragment namespace*/
  _showFluted = BoolOptionValue("show_fluted", "", false);
  _showFluted.description = "Show how fluted resolver works";
  _lookup.insert(&_showFluted);
  _showFluted.tag(OptionTag::DEVELOPMENT);
  ...
  auto choiceArithmeticSimplificationMode = [&](std::string l, std::string s, ArithmeticSimplificationMode d) { return ChoiceOptionValue<ArithmeticSimplificationMode>(l, s, d, {
                                                                                                                                                                                    "force",
                                                                                                                                                                                    "cautious",
                                                                                                                                                                                    "off",
                                                                                                                                                                                }); };
  ...
  _splittingCongruenceClosure.addHardConstraint(If(equal(SplittingCongruenceClosure::MODEL)).then(_splittingMinimizeModel.is(notEqual(SplittingMinimizeModel::SCO))));
  ...
  _satSolver = ChoiceOptionValue<SatSolver>("sat_solver", "sas", SatSolver::MINISAT, {"minisat"
#if VZ3
                                                                                      ,
                                                                                      "z3"
#endif
...
{
  float newValue;
  if (!Int::stringToFloat(value.c_str(), newValue))
    return false;
...
void Options::readOptionsString(std::string optionsString, bool assign)
{
  ...
  while (optionsString != "") {
    ...
    error:
      USER_ERROR("bad option specification '" + optionsString + "'");
    }
...
void Options::readFromEncodedOptions(std::string testId)
{
  ...
  error:
    USER_ERROR("bad test id " + _testId.actualValue);
  }
...
{
  if (_forcedOptions.actualValue.empty())
    return;
...
std::string Options::generateEncodedOptions() const
{
  ...
  switch (_saturationAlgorithm.actualValue) {
    case SaturationAlgorithm::LRS:
      sat = "lrs";
      break;
    case SaturationAlgorithm::DISCOUNT:
      sat = "dis";
      break;
    case SaturationAlgorithm::OTTER:
      sat = "ott";
      break;
    case SaturationAlgorithm::FINITE_MODEL_BUILDING:
      sat = "fmb";
      break;
    default:
      ASSERTION_VIOLATION;
...
bool Options::complete(const Problem &prb) const
{
  ...
  if (prop.hasInterpretedOperations() || prop.hasProp(Property::PR_HAS_INTEGERS) || prop.hasProp(Property::PR_HAS_REALS) || prop.hasProp(Property::PR_HAS_RATS) || prop.hasProp(Property::PR_HAS_ARRAYS) || (!prop.onlyFiniteDomainDatatypes() && prop.hasProp(Property::PR_HAS_DT_CONSTRUCTORS)) || (!prop.onlyFiniteDomainDatatypes() && prop.hasProp(Property::PR_HAS_CDT_CONSTRUCTORS)) || prop.hasAnswerLiteral()) {
    return false;
  }
  ...
  if (!unitEquality) {
    if (_selection.actualValue <= -1000 || _selection.actualValue >= 1000)
      return false;
    if (_literalComparisonMode.actualValue == LiteralComparisonMode::REVERSE)
      return false;
  }

  if (!hasEquality) {
    if (_binaryResolution.actualValue)
      return true;
    // binary resolution is off
    if (_unitResultingResolution.actualValue != URResolution::FULL &&
        (_unitResultingResolution.actualValue != URResolution::ON || _splitting.actualValue))
      return false;
